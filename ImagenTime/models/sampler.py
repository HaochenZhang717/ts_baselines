...
class DiffusionProcess():
    ...
    @torch.no_grad()
    def sampling(self, sampling_number=16, impute=False, xT=None, context=None):
        if xT is None:
...
class MultiModalDiffusionProcess():
    def __init__(self, args, diffusion_fn, shape):
        '''
        beta_1        : beta_1 of diffusion process
        beta_T        : beta_T of diffusion process
    ...
    def impute(self, x, latents, mask, context):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            # Euler step.
            # denoised = self.net(x_hat.float(), t_hat.float(), context.float()).to(torch.float64)
            ...
            # Apply 2nd order correction.
            if i < self.num_steps - 1:
                # denoised = self.net(x_next.float(), t_next.float(), context.float()).to(torch.float64)
    ...
    @torch.no_grad()
    def interpolate(self, x, mask, xT, context):
        return self.impute(x, xT, mask, context)
