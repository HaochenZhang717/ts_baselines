...
from models.networks import EDMPrecond, EDMPrecondLDM, MultimodalEDMPrecond
...
class MultimodalImagenTime(nn.Module):
    def __init__(self, args, device):
        '''
        beta_1    : beta_1 of diffusion process
        beta_T    : beta_T of diffusion process
    ...
    def loss_fn_impute(self, x, mask, context):
        '''
        x          : real data if idx==None else perturbation data
        idx        : if None (training phase), we perturbed random index.
        '''
        ...
        to_log = {}
        output, weight = self.forward_impute(x, mask, context)
        x = self.unpad(x * (1 - mask), x.shape)
        output = self.unpad(output * (1 - mask), x.shape)
        loss = (weight * (output - x).square()).mean()
        to_log['karras loss'] = loss.detach().item()
    ...
    def forward_impute(self, x, mask, context, augment_pipe=None):
        ...
        # clear image
        x = x * mask
        D_yn = self.net(x + x_to_impute, sigma, context, augment_labels=None)
        return D_yn, weight
...
class TextImagenTime(nn.Module):
    def __init__(self, args, device):
        '''
        beta_1    : beta_1 of diffusion process
        beta_T    : beta_T of diffusion process
        ...
        self.net = EDMPrecond(
            args.img_resolution,
            args.input_channels,
            model_type=args.model_type if args.model_type is not None else 'DhariwalUNet',
...
class TextLDM(nn.Module):
    def __init__(self, args, device):
        ...
        self.net = EDMPrecondLDM(
            ...
            num_head_channels=args.num_heads_channels,
