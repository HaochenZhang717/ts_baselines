...
from models.encoders.text_encoder import TextEncoder, CLIPTextEncoderV6
from models.encoders.cond_projector import TextProjectorMVarMScaleMStep, AttrProjectorAvg
from models.unconditional_generator_v6 import UnConditionalGeneratorV6
from models.cttp.cttp_model import CTTP
import time
import random
import yaml

class ConditionalGeneratorV6(nn.Module):
    ...
    def _init_condition_encoders(self, diff_configs, cond_configs):
        if cond_configs["cond_modal"] == "constraint":
            ...
            if cond_configs["cond_modal"] == "text":
                ...
                self.attr_en = CLIPTextEncoderV6(cond_configs["text"]).to(self.device)
    ...
    def _init_diff(self, configs):
        ...
        self.generator = UnConditionalGeneratorV6(configs=configs)
    ...
    def _unpack_data_cond_gen(self, batch):
        ...
        ts = ts.permute(0, 2, 1)
        B, _, _, D = batch['cap_0323_embed'].shape
        return ts, tp, batch['cap_0323_embed'].reshape(B, -1, D).to(self.device).float()
