...
class ResidualBlock(nn.Module):
    def __init__(self, side_dim, channels, diffusion_embedding_dim, nheads, condition_type="add"):
        ...
        if condition_type == "add":
            ...
            self.adaLN_modulation_vae = nn.Sequential(
                nn.SiLU(),
    ...
    def forward(self, x, side_emb, vae_embed, text_embed, diffusion_emb, attention_mask=None, condition_type="add"):
    
        if condition_type == "adaLN":
            gama_vae, beta_vae, alpha_vae = self.adaLN_modulation_vae(vae_embed.permute(0,2,3,1)).chunk(3, dim=-1)
            gama_vae, beta_vae, alpha_vae = gama_vae.permute(0,3,1,2), beta_vae.permute(0,3,1,2), alpha_vae.permute(0,3,1,2)

            gama_text, beta_text, alpha_text = self.adaLN_modulation_text(text_embed.permute(0, 2, 3, 1)).chunk(3, dim=-1)
            gama_text, beta_text, alpha_text = gama_text.permute(0, 3, 1, 2), beta_text.permute(0, 3, 1,2), alpha_text.permute(0, 3,1, 2)
        ...
        y = x + diffusion_emb
        if condition_type == "adaLN":
            y = self.modulate(y, gama_vae, beta_vae)
            y = self.modulate(y, gama_text, beta_text)
...
class HybridTS(nn.Module):
    ...
    def forward(self, x_raw, tp, vae_embed_raw, text_emb_raw, diffusion_step):
        ...
        if vae_embed_raw is None:
            vae_embed = torch.zeros_like(x_in)
        else:
            if "scale" in self.config["text_projector"]:
                assert len(scale_length) == vae_embed_raw.shape[2]
                mscale_attr_list = []
                for i in range(len(scale_length)):
                    tmp_scale_attr = vae_embed_raw[:,:,i:i+1,:].expand([-1, -1, scale_length[i], -1])
                    mscale_attr_list.append(tmp_scale_attr)
                vae_embed = torch.cat(mscale_attr_list, dim=2)
            else:
                vae_embed = vae_embed_raw.expand([-1, -1, x_in.shape[-1], -1])
            vae_embed = vae_embed.permute(0,3,1,2)


        if text_emb_raw is None:
            text_embed = torch.zeros_like(x_in)
        else:
            text_emb_raw = text_emb_raw.permute(0, 3, 2, 1)
            text_embed_list = []
            for i in reversed(range(self.multipatch_num)):
                patch_length = self.config["base_patch"] * self.config["L_patch_len"] ** i
                text_embed_list.append(text_emb_raw.repeat_interleave(32 // patch_length, dim=-1))
            text_embed = torch.cat(text_embed_list, dim=-1)
        ...
        for layer in self.residual_layers:
            x_in, skip_connection = layer(x_in+_x_in, side_in, vae_embed, text_embed, diffusion_emb, attention_mask=None, condition_type=self.config["condition_type"])
