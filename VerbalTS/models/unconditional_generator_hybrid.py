...
from models.diffusion.hybridts import HybridTS
...
class UnConditionalGeneratorHybrid(nn.Module):
    ...
    def _init_diff(self, configs):
        ...
        self.diff_model = HybridTS(configs, inputdim=1).to(self.device)
    ...
    def _noise_estimation_loss(self, x, tp, vae_embed,text_embed, t):
        noise = torch.randn_like(x)
        noisy_x = self.ddpm.forward(x, t, noise)
        pred_noise, loss_dict = self.predict_noise(noisy_x, tp, vae_embed, text_embed, t)
    ...
    def predict_noise(self, xt, tp, vae_attr_embed, text_embed, t):
        noisy_x = torch.unsqueeze(xt, 1)
        pred_noise, loss_dict = self.diff_model(noisy_x, tp, vae_attr_embed, text_embed, t)
